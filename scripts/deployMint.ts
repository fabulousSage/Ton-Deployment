...
export async function run(provider: NetworkProvider) {
    ...
    const jettonMinter = provider.open(
        JettonMinter.createFromAddress(Address.parse('EQDoFf4tXBYR_61fhUIQuEE2ppKedJJRJv4mlADX2BlysG-g')),
    );
    ...
    const res = await jettonMinter.sendMint(sender, mintAddress, nanoMint, toNano('0.01'), toNano('0.02'));
